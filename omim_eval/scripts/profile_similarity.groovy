...
def omim = new File('data/associations/omim_ihprf3_anns.txt').text.split('\n').collect { it.split('\t')[0] }
def multivariate = new File('data/associations/multivariate_klarigi.tsv').text.split('\n').collect { it.split('\t')[0] }
def univariate = new File('data/associations/univariate_klarigi.tsv').text.split('\n').collect { it.split('\t')[0] }
def binomial = new File('data/binomial.sigResults_enriched.tsv').text.split('\n').collect { it.split('\t').last() }.findAll { it =~ 'HP' }
def fisher = new File('data/fisher.sigResults_enriched.tsv').text.split('\n').collect { it.split('\t').last() }.findAll { it =~ 'HP' }
...
println 'Fisher: ' + engine.compare(smConfGroupwise, smConfPairwise, omim.collect { getURIfromTerm(it) }.toSet(), fisher.collect { getURIfromTerm(it) }.toSet())
println 'Binomial: ' + engine.compare(smConfGroupwise, smConfPairwise, omim.collect { getURIfromTerm(it) }.toSet(), binomial.collect { getURIfromTerm(it) }.toSet())
println 'Univariate: ' + engine.compare(smConfGroupwise, smConfPairwise, omim.collect { getURIfromTerm(it) }.toSet(), univariate.collect { getURIfromTerm(it) }.toSet())
println 'Multivariate: ' + engine.compare(smConfGroupwise, smConfPairwise, omim.collect { getURIfromTerm(it) }.toSet(), multivariate.collect { getURIfromTerm(it) }.toSet())
